...
function [AGVs, task_list] = build_case_agvs(case_cfg, cell_id_map)
    ...
    for i = 1:numel(agv_fields)
        ...
        AGVs = [AGVs, init_case_agv(agv_data, cell_id_map)]; %#ok<AGROW>
        task_list(end + 1, :) = [agv_data.active_task_id, 1, max(1, agv_data.payload_weight), 120]; %#ok<AGROW>
    end
end

function agv = init_case_agv(agv_case, cell_id_map)
...
function result = simulate_case_event_driven_smallmap(case_cfg, grid_map, cell_id_map, cfg)
    ...
    for id = 1:num_agvs
        % Normalize every seeded case path before any reservation/conflict
        % reasoning. Test cases may contain coarse IDs (e.g. 233 -> 231) or
        % repeated IDs (e.g. 231 -> 231), which should not create synthetic
        % early arrivals or phantom holds in the event-window detector.
    ...
    traces = repmat(struct('cells', [], 'times', [], 'tags', {{}}), 1, num_agvs);
    for id = 1:num_agvs
    ...
    result.priority_snapshot_str = format_priority_snapshot_smallmap(result.priority_snapshot);
    result.priority_pairwise_str = format_pairwise_priority_smallmap(result.priority_snapshot, 1:numel(AGVs));
    ...
    while event_count < max_events
        ...
        if ~isempty(cycle_nodes)
            ...
            result.debug_logs{end + 1, 1} = sprintf('  interlock_breaker[t=%g]: breaker=AGV%d blocker=AGV%d', ...
                current_t, breaker_id, breaker_blocker_id); %#ok<AGROW>
        ...
        for rec_idx = 1:numel(conflict_records)
            ...
            if ~isempty(cycle_nodes) && rec.self_id == breaker_id
                ...
                result.debug_logs{end + 1, 1} = sprintf('  interlock_break_action[t=%g]: AGV%d -> %s', ...
                    current_t, breaker_id, strategy_name); %#ok<AGROW>
            ...
            if is_moving_state_local(AGVs(id).status)
                ...
                if post_blocker_id > 0
                    ...
                    result.debug_logs{end + 1, 1} = sprintf( ...
                        ...
                        current_t, id, post_blocker_id); %#ok<AGROW>
                    continue;
                end
    ...
    function handle_waiting_smallmap(id, event_t)
        ...
        if ismember(AGVs(id).status, {'Loading', 'Unloading'})
        ...
        % non-moving occupied states keep reserving current node
        AGVs(id).next_event_t = event_t + 1;
        append_trace(id, AGVs(id).pos, display_event_time(event_t, AGVs(id).step_dur), 'hold');
    end
    ...
    function path_rc = normalize_path_four_connected(path_rc, payload_weight, agv_type)
        ...
        for seg_idx = 2:size(path_rc, 1)
            ...
            if isequal(curr_node, prev_node)
                % Consecutive duplicate IDs in handcrafted cases should not
                % become fake move/hold events.
                continue;
            end
...
function [debug_lines, cycle_nodes, breaker_id, breaker_blocker_id, breaker_debug] = build_interlock_debug_lines(AGVs, conflict_records, current_t, task_list)
    ...
    for id = 1:num_agvs
        ...
        edge_parts{end + 1} = sprintf('AGV%d->[%s]{%s}', id, sprintf('AGV%d ', edges{id}), reason_txt); %#ok<AGROW>
    ...
    for i = 1:numel(sccs)
        ...
        cycle_parts{end + 1} = sprintf('{%s}', sprintf('AGV%d ', comp)); %#ok<AGROW>
...
function txt = format_cycle_state_smallmap(AGVs, cycle_nodes, task_list, current_t)
    ...
    for i = 1:numel(cycle_nodes)
        ...
        parts{i} = sprintf('AGV%d[state=%s,pos=[%d,%d],next_t=%g,target=[%d,%d],p=%.3f]', ...
            id, AGVs(id).status, AGVs(id).pos(1), AGVs(id).pos(2), AGVs(id).next_event_t, ...
...
function [breaker_id, debug_str] = select_interlock_breaker_smallmap(AGVs, cycle_nodes, task_list, current_t)
    ...
    for i = 1:numel(cycle_nodes)
        ...
        parts{i} = sprintf('AGV%d[p=%.3f,payload=%g,score=%.4f]', id, p, AGVs(id).payload_weight, score);
        if score < best_score
            best_score = score;
            breaker_id = id;
        end
...
function txt = format_priority_snapshot_smallmap(priority_values)
    ...
    for i = 1:numel(priority_values)
        parts{i} = sprintf('AGV-%d=%.3f', i, priority_values(i));
    end
    txt = strjoin(parts, ' | ');
end

function txt = format_pairwise_priority_smallmap(priority_values, agv_ids)
    txt = '';
...
function case_timeline = build_runtime_timeline_rows(case_name, traces, cell_id_map, winner_id, loser_id)
    ...
    for agv_id = 1:numel(traces)
        ...
        for k = 1:numel(dense_times)
            rc = dense_cells(k, :);
            case_timeline(end + 1, :) = { ...
                case_name, agv_id, role, dense_times(k), ...
...
function plot_case_runtime_visualization(grid_map, ~, ~, result, output_png)
    ...
    for agv_id = 1:numel(runtime_traces)
        ...
        text(ax, tr.cells(1, 2) + 0.2, tr.cells(1, 1) - 0.2, sprintf('AGV-%d', agv_id), 'Color', palette(agv_id, :), 'FontWeight', 'bold');
    end
...
function traces = reconstruct_traces_from_rows(case_timeline)
    ...
    traces = repmat(struct('cells', [], 'times', [], 'tags', {{}}), 1, numel(agv_ids));
    for i = 1:numel(agv_ids)
        agv_id = agv_ids(i);
...
function plot_case_visualization(grid_map, ~, case_cfg, AGVs, ~, ~, ...
    ...
    for agv_id = 1:numel(AGVs)
        draw_agv_case(ax, AGVs(agv_id), palette(agv_id, :), sprintf('AGV-%d', agv_id));
    end
...
function case_timeline = build_case_timeline_rows(case_name, AGVs, cell_id_map, current_t, winner_id, loser_id, strategy_name, replanned_path, strategy_info)
    ...
    for agv_id = 1:numel(AGVs)
        ...
        for k = 1:numel(timeline_times)
            rc = timeline_cells(k, :);
            case_timeline(end + 1, :) = { ...
                case_name, agv_id, role, timeline_times(k), ...

function ga_text_scirpt_for_lift()   
    ...
    % =========================================================
    % 【接口适配】：使用极简的结构体 [schedules, batches, metrics, history] 接收
    % =========================================================
    ...
    for i = 1:length(batch_exp)
        if agv_types(i) == 2 
            fprintf('AGV %d (叉车): 采用单件串行模式，无批次信息，请直接查看 schedules\n', i);
            fprintf('--------------------------------------------------\n');
            continue;
        end
    ...
    % =========================================================
    % 绘制：总能耗 (Energy) 对比图 (原为载重积分)
    % =========================================================
    figure('Name', '托举式 AGV 物理能耗对比', 'Color', 'w', 'Position', [100, 100, 700, 500]);
    ...
    ylabel('系统总能耗 (Energy / 相对单位)', 'FontSize', 12); % 【修改】Y轴更名
    grid on; set(gca, 'GridAlpha', 0.3);
    legend('Location', 'northeast'); % 【修改】能耗变小，图例放右上角
    axis tight;

    % =========================================================
    % 绘制：最大完工时间 (Time) 对比图
    % =========================================================
    figure('Name', '托举式 AGV 完工时间对比', 'Color', 'w', 'Position', [150, 150, 700, 500]);
    ...
    % 绘制：总距离 (Distance) 对比图
    % =========================================================
    figure('Name', '托举式 AGV 算法性能对比', 'Color', 'w', 'Position', [200, 200, 700, 500]);
    ...
    ylabel('最大行驶时间/s', 'FontSize', 12);
    zlabel('最小总能耗/J', 'FontSize', 12);
    title('(a) NSGA-II 算法', 'FontSize', 12);
    ...
    % 大标题使用防报错格式
    sgtitle('NSGA-II 算法与改进 NSGA-II 算法 Pareto 解集 (托举式AGV)', 'FontSize', 14, 'FontWeight', 'bold');

    evaluate_and_plot_moea(hist_exp.lift.gen_fronts, hist_base.lift.gen_fronts);
    %% 6. 打印全局基因序列 (Schedules)
    fprintf('\n================ 全局调度任务序列 (Schedules) ================\n');
    for k = 1:num_agvs
        type_str = '未知';

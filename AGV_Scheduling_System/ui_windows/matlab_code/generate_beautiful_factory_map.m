function generate_beautiful_factory_map()
    ...
    fprintf('\n>> [地图监控] 正在展开分散式大件仓库(ID:13-16)对应的 9 栅格详细坐标：\n');
    block_w = 3; block_h = 3;
    ...
    fprintf('\n>> [地图监控] 正在展开中间 U 型配件工位(ID:1-12)对应的详细栅格坐标：\n');
    ...
    fprintf('\n>> [地图监控] 正在展开左下配件   仓库(ID:1-12)对应的详细栅格坐标：\n');
    lb_start_x = 3; lb_top_y = 18;
    lb_bot_y = 10;
    % 假设 box_w 和 box_h 已在外部定义，通常为 2
    ...
    fprintf('\n>> [地图监控] 正在展开分散式大件工位(ID:13-16)对应的 9 栅格详细坐标：\n');
    block_w = 3; block_h = 3;
    ...
    % 标题
    title('转向架组装生产工厂栅格图', 'FontSize', 14, 'FontWeight', 'bold', 'Color', [0.2 0.2 0.2]);
end
...
function draw_legend_panel(x, y, colors, labels)
    ...
    text(x + box_w/2, y + box_h - 1.5, '图 例 / Legend', 'FontSize', 11, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');

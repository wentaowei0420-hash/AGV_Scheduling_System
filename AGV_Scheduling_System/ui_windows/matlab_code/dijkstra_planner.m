function [path, gScore_goal, turn_count, expanded_nodes, path_length, gScore_matrix] = dijkstra_planner(map, start, goal)
    % Dijkstra路径规划函数
    % 输入：
    ...
    % 初始化数据结构
    openSet = start;                     % 待探索节点列表
    cameFrom = zeros(mapSize(1), mapSize(2), 2);  % 父节点记录
    gScore = inf(mapSize);                % 从起点到每个节点的最短代价
    ...
    while ~isempty(openSet)
        % 从openSet中找到gScore最小的节点（线性索引加速）
        linearIdx = openSet(:,1) + (openSet(:,2)-1) * mapSize(1);
        [~, currentIdx] = min(gScore(linearIdx));

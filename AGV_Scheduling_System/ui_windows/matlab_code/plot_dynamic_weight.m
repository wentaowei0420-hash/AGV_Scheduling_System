...
% 绘制主曲线（标准学术蓝）
plot(dist_current_to_goal, w_n, '-', 'Color', '#0072BD', 'LineWidth', 1, 'DisplayName', '动态权重 w(n)');
hold on;

% 绘制基准线（标准 A* 权重）
yline(1.0, '--', 'Color', '#7E2F8E', 'LineWidth', 1, 'DisplayName', '标准 A* 权重 (w=1)');

% 绘制关键节点（采用空心标记，稳重不花哨）
plot(dist_start_to_goal, w_n(1), 's', 'MarkerSize', 7, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', '#D95319', 'LineWidth', 1, 'DisplayName', '起点 (偏向贪心搜索)');
plot(0, w_n(end), 'o', 'MarkerSize', 7, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', '#0072BD', 'LineWidth', 1, 'DisplayName', '终点 (回归精确寻优)');

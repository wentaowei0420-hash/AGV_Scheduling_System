function Test_Map_Vision()
    ...
    % 创建图形窗口
    figure('Name', '电脑眼中的栅格地图 (含网格边界)', 'Color', 'w', 'Position', [100, 200, 1200, 600]);
    
    %% === 场景 1: 托举式 AGV (Target ID = 5, 小配件) ===
    target_id_small = 17;
    % 生成地图
    ...
    title(['场景A: 托举式 AGV (去目标 ' num2str(target_id_small) ')']);
    subtitle('注意右下角：叉车基地已被封锁 (黑色)');
    
    % 用红框圈出重点验证区域 (叉车基地 x=39~49, y=2)
    hold on;
    rectangle('Position', [39, 2, 10, 3], 'EdgeColor', 'r', 'LineWidth', 2, 'LineStyle', '--');
    text(39, 7, '叉车基地 (已封锁)', 'Color', 'r', 'FontSize', 10, 'FontWeight', 'bold');
    
    % 圈出目标 (应该可以看到目标5号是白的)
    rectangle('Position', [19, 18, 2, 2], 'EdgeColor', 'g', 'LineWidth', 2);
    ...
    subtitle('注意左下角：托举基地已被封锁 (黑色)');
    
    % 用红框圈出重点验证区域 (托举基地 x=2~14, y=2)
    hold on;
    rectangle('Position', [2, 2, 14, 3], 'EdgeColor', 'r', 'LineWidth', 2, 'LineStyle', '--');
    text(2, 7, '托举基地 (已封锁)', 'Color', 'r', 'FontSize', 10, 'FontWeight', 'bold');
    
    % 圈出目标
    rectangle('Position', [4, 41, 3, 3], 'EdgeColor', 'g', 'LineWidth', 2);
...
function show_map(gridMap)
    ...
    for x = 0.5 : 1 : cols + 0.5
        line([x, x], [0.5, rows + 0.5], 'Color', [0.6 0.6 0.6], 'LineWidth', 0.1);
    end
    
    % B. 画横线 (Horizontal Lines)
    % 从 0.5 开始，步长为 1，画到最上边
    for y = 0.5 : 1 : rows + 0.5
        line([0.5, cols + 0.5], [y, y], 'Color', [0.6 0.6 0.6], 'LineWidth', 0.1);
    end
end